...
t[#t+1]=Def.ActorFrame{
	...
	--	InitCommand=cmd(diffuse,ColorDarkTone(PlayerColor(PLAYER_1)));
	...
	Def.GrooveRadar {
		...
		ChangedCommand=function(self)
			...
			if song then
				...
				for i=1,5 do
					if smexp_gr=="" then
						radar_x[i]=yaGetRadarVal(song,PLAYER_1,RadarPrm[i],false) *l*math.sin(2*math.pi*(i-1)/5);
						radar_y[i]=yaGetRadarVal(song,PLAYER_1,RadarPrm[i],false) *l*math.cos(2*math.pi*(i-1)/5)*(-1);
					else
				...
				for i=1,5 do
				radar_x[i+5]= yaGetRadarVal(song,PLAYER_1,RadarPrm[i],true) *l*math.sin(2*math.pi*(i-1)/5);
				radar_y[i+5]= yaGetRadarVal(song,PLAYER_1,RadarPrm[i],true) *l*math.cos(2*math.pi*(i-1)/5)*(-1);
				end;
		...
		CurrentStepsP1ChangedMessageCommand=cmd(playcommand,"Changed");
		CurrentTrailP1ChangedMessageCommand=cmd(playcommand,"Changed");
...
t[#t+1]=Def.ActorFrame{
	...
	PlayerJoinedMessageCommand=cmd(player,PLAYER_1;);
	LoadActor("../_GrooveRadar autogen")..{
		...
		OffCommand=cmd(sleep,0.183;decelerate,0.167;zoom,0);
		CurrentStepsP1ChangedMessageCommand=cmd(playcommand,"Joined");
		CurrentTrailP1ChangedMessageCommand=cmd(playcommand,"Joined");
	...
	Def.Quad{
		...
		ChangedCommand=function(self)
			if GAMESTATE:GetCurrentSong() then
				...
				if i<=5 then
					self:diffuse(PlayerColor(PLAYER_1));
					self:diffuseleftedge(1,0.3,0,1);
					self:diffusealpha(1);
