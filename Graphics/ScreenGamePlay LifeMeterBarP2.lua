...
frame=Def.ActorFrame{
	...
	Def.Quad{
		...
		OffCommand=function(self)
			if SCREENMAN:GetTopScreen():GetLifeMeter(PLAYER_2):GetLife()<=0 then
...
local function update(self)
	...
	local lifestate;
	glm=SCREENMAN:GetTopScreen():GetLifeMeter(PLAYER_2);
	...
	local beat = GetPlayerSongBeat2(PLAYER_2,msec);
		if haishin=="On" then
			beat=0.0001;
		end;
	...
	nlifes:zoomtowidth(lifeWidth*life_d);
	if lifestate==1 then
	-- Hot
