...
t[#t+1] = Def.Quad {
	InitCommand=cmd(Center;zoomto,SCREEN_WIDTH,SCREEN_HEIGHT;diffuse,0,0,0,1);
...
t[#t+1] = Def.Sprite {
	...
	ChangeCourseSongInMessageCommand=function(self)
		...
		if haishin=="On" then
			...
			self:sleep(1.7);
			self:linear(0.3);
			self:scaletofit( 0,0,SCREEN_HEIGHT*4/6,SCREEN_HEIGHT/2 );
			self:y(SCREEN_CENTER_Y);
			if GAMESTATE:IsPlayerEnabled(PLAYER_1) then
				self:x(SCREEN_WIDTH*3/4-50);
				self:rotationy(45);
			else
				self:x(SCREEN_WIDTH*1/4+50);
				self:rotationy(-45);
			end;
		else
			self:scale_or_crop_background();
